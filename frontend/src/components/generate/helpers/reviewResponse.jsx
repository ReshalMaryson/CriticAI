...
export default function ReviewResponse({ review, setReview, keyword }) {
  return (
    <div className="review-result-container">
      {/* modified for keyword */}
      {keyword ? null : (
        <button
          className="back-btn"
          onClick={() => {
      ...
      <div className="result-grid">
        <div className="result-card score-card">
          <h2>Score</h2>
          <h1 className="score-value">{review.score} / 100</h1>
        </div>
        ...
        <div className="result-card">
          ...
          {review.confirmedIssues.length > 0 ? (
            review.confirmedIssues.map((item, index) => (
              <p key={index}>• {item}</p>
            ))
          ) : (
            <>
